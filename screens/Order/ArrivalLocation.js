...
function ArrivalLocation ({ navigation }) {
  ...
  const { setMapLocationForArrival } = context
    ...
    if (!street) {
      Geocoder.from(truckLocation?.latitude, truckLocation?.longitude)
        .then(async json => {
          ...
          const payload = {
            arrival_address_street_one: addressComponent.short_name,
        ...
        {isSearch ? (
          ...
          >
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <Text style={styles.title}>{'Choose Arrival Address'}</Text>
            </View>
            <TouchableOpacity
