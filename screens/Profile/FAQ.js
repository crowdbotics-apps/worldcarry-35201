...
function FAQ({ navigation, route }) {
  ...
  const filtered = (key, value) => {
    ...
    if (value) {
      ...
      handleChange("filteredList", FAQs)
    }
  }

  const filtered1 = (key, value) => {
    handleChange(key, value)
    if (value) {
      const newData = questions?.filter(function (item) {
      ...
      handleChange("filteredList1", newData)
    } else {
      handleChange("filteredList1", questions)
    }
  }
    ...
    <View style={styles.container}>
      ...
      <View style={{ width: "90%", marginTop: 10, marginBottom: 15 }}>
        <AppInput
          ...
          value={searchText}
          name={"searchText"}
          onChange={questions?.length > 0 ? filtered1 : filtered}
      ...
      {questions?.length > 0 ? (
        ...
        <>
          {filteredList.map((item, index) => (
            <TouchableOpacity
              onPress={() => {
                handleChange("questions", item?.QAlist)
                handleChange("filteredList1", item?.QAlist)
              }}
              key={index}
