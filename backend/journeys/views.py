...
class JourneyOrderRequest(APIView):
    ...
    def post(self, request):
        ...
        if user == 'sender':
            try:
                create_notification({
                    "name": f"Shipper {request.user.name} requested a consignment for delivery.",
                    "description": f"Shipper {request.user.name} requested a consignment for delivery.",
                    "user": journey.user,
            ...
            try:
                create_notification({
                    "name":f"Carrier {request.user.name} requested to deliver {order.product_name}.", 
                    "description": f"Carrier {request.user.name} requested to deliver {order.product_name}.",
                    "user": order.user, 
                    "object_id":order.id, 
